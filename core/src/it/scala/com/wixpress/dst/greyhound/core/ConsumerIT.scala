...
class ConsumerIT extends BaseTestWithSharedEnv[Env, TestResources] {
  ...
  s"subscribe to a pattern" in
    ZIO.scoped {
      for {
        _                             <- ZIO.debug(">>>> starting test: patternTest with parallel")
        ...
        _ <- makeConsumer(kafka, compile("core-subscribe-pattern1.*"), group, handler, 0, useParallelConsumer = false).flatMap { consumer =>
  ...
  s"subscribe to a pattern - using parallel consumer" in
    ZIO.scoped {
      for {
        _                             <- ZIO.debug(">>>> starting test: patternTest with parallel")
        topic1                         = "core-subscribe-parallel-pattern1-topic"
        topic2                         = "core-subscribe-parallel-pattern2-topic"
        r                             <- getShared
        TestResources(kafka, producer) = r
        _                             <- kafka.createTopics(Seq(topic1, topic2).map(t => TopicConfig(t, 1, 1, delete)): _*)
        group                         <- randomGroup
  ...
  Fragments.foreach(Seq(false, true)) { useParallelConsumer =>
    s"produce, consume and rebalance${parallelConsumerString(useParallelConsumer)}" in
      ZIO.scoped {
        for {
          r                             <- getShared
          TestResources(kafka, producer) = r
    ...
    s"be able to resubscribe to same topics${parallelConsumerString(useParallelConsumer)}" in
      ZIO.scoped {
        for {
          r                             <- getShared
          TestResources(kafka, producer) = r
          ...
          config  =
            configFor(
              ...
              group,
              topic,
              mutateEventLoop = _.copy(drainTimeout = 1.second, consumePartitionInParallel = useParallelConsumer, maxParallelism = 8)
            ).copy(clientId = cId)
          record  = ProducerRecord(topic, "bar", Some("foo"))
    ...
    s"produce consume null values (tombstones)${parallelConsumerString(useParallelConsumer)}" in
      ZIO.scoped {
        for {
          r                             <- getShared
          TestResources(kafka, producer) = r
    ...
    s"not lose any messages on a slow consumer (drives the message dispatcher to throttling)${parallelConsumerString(useParallelConsumer)}" in
      ZIO.scoped {
        for {
          r                             <- getShared
          TestResources(kafka, producer) = r
    ...
    s"delay resuming a paused partition${parallelConsumerString(useParallelConsumer)}" in
      ZIO.scoped {
        for {
          r                             <- getShared
          TestResources(kafka, producer) = r
    ...
    s"pause and resume consumer${parallelConsumerString(useParallelConsumer)}" in
      ZIO.scoped {
        for {
          r                             <- getShared
          TestResources(kafka, producer) = r
    ...
    s"consumer from earliest offset${parallelConsumerString(useParallelConsumer)}" in
      ZIO.scoped {
        for {
          r                             <- getShared
          TestResources(kafka, producer) = r
    ...
    s"consumer from a new partition is interrupted before commit (offsetReset = Latest)${parallelConsumerString(useParallelConsumer)}" in
      ZIO.scoped {
        for {
          r                             <- getShared
          TestResources(kafka, producer) = r
          ...
          _                             <-
            makeConsumer(
              ...
              hangingHandler,
              0,
              mutateEventLoop = _.copy(drainTimeout = 200.millis, consumePartitionInParallel = useParallelConsumer, maxParallelism = 8)
            )
              .flatMap { consumer =>
          ...
          consumedValues <- makeConsumer(
                              ...
                              1,
                              mutateEventLoop = _.copy(consumePartitionInParallel = useParallelConsumer, maxParallelism = 8),
                              modifyConfig = _.copy(offsetReset = Latest)
                            )
    ...
    s"block until current tasks complete${parallelConsumerString(useParallelConsumer)}" in
      ZIO.scoped {
        for {
          r                             <- getShared
          TestResources(kafka, producer) = r
    ...
    s"rewind positions on poll failure${parallelConsumerString(useParallelConsumer)}" in
      ZIO.scoped {
        ...
        for {
          ...
          decryptor: BinaryDecryptor     = new BinaryDecryptor {
                                             override def decrypt(record: ConsumerRecord[Chunk[Byte], Chunk[Byte]])(
                                               implicit trace: Trace
                                             ): ZIO[Any, RuntimeException, ConsumerRecord[Chunk[Byte], Chunk[Byte]]] =
  ...
  private def makeConsumer(
    ...
    RecordConsumer.make(
      configFor(kafka, group, pattern, useParallelConsumer).copy(clientId = s"client-$i", offsetReset = OffsetReset.Earliest),
      handler
    )
  ...
  private def configFor(
    ...
    RecordConsumerConfig(
      ...
      extraProperties = fastConsumerMetadataFetching,
      eventLoopConfig = EventLoopConfig.Default.copy(consumePartitionInParallel = useParallelConsumer, maxParallelism = 8)
    )
  ...
  private def parallelConsumerString(isParallel: Boolean) =
    if (isParallel) " - using parallel consumer" else ""
