...
import com.wixpress.dst.greyhound.core.metrics.GreyhoundMetrics.GreyhoundMetrics
...
class LocalBufferProducerIT extends BaseTestWithSharedEnv[ITEnv.Env, BufferTestResources] {
    ...
    for {
      ...
      test <- Producer.make(failFastInvalidBrokersConfig).use { producer =>
        for {
          ...
          timeoutCount <- makeProducer(producer, maxConcurrency = 10).use { localBufferProducer =>
            for {
              _ <- localBufferProducer.produce(record(topic), IntSerde, StringSerde).flatMap(_.kafkaResult).timeout(10.second)
              timeouts <- TestMetrics.reported.map(_.collect { case e@LocalBufferProduceAttemptFailed(TimeoutError(_), false) => e })
            } yield timeouts.size
          }
        } yield timeoutCount must beGreaterThan(1)
      }
    } yield test
  }
  ...
  private def makeProducer(producer: Producer,
                           ...
                           giveUpAfter: Duration = 1.day): ZManaged[ZEnv with GreyhoundMetrics, Throwable, LocalBufferProducer] =
    makeH2Buffer.flatMap(buffer =>
      LocalBufferProducer.make(producer, buffer, LocalBufferProducerConfig(maxConcurrency = maxConcurrency,
        maxMessagesOnDisk = maxMessagesOnDisk, giveUpAfter = giveUpAfter)))
