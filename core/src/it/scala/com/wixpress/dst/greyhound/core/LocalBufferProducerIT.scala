...
class LocalBufferProducerIT extends BaseTestWithSharedEnv[ITEnv.Env, BufferTestResources] {
    ...
    for {
      ...
      test <- Producer.make(failFastInvalidBrokersConfig).use { producer =>
        (for {
          ...
          timeoutCount <- makeProducer(producer, maxConcurrency = 10, flushTimeout = 1.second).use { localBufferProducer =>
            for {
          ...
          console.putStrLn("Closing producer...")
      }
    } yield test
  }
  ...
  private def makeProducer(producer: Producer,
                           ...
                           flushTimeout: Duration = 1.minute): ZManaged[ZEnv with GreyhoundMetrics, Throwable, LocalBufferProducer] =
