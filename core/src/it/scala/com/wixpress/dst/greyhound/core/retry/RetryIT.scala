...
class RetryIT extends BaseTestWithSharedEnv[Env, TestResources] {
  ...
  "commit message on failure with NonRetryableException" in {
    for {
      ...
      invocations <- RecordConsumer.make(configFor(kafka, group, retryConfig, topic), retryHandler).use_ {
        producer.produce(ProducerRecord(topic, "bar", Some("foo")), StringSerde, StringSerde) *> invocations.get.delay(2.seconds)
      }
  ...
  "configure a regex consumer with a retry policy" in {
    for {
      ...
      success <- RecordConsumer
        ...
        .use_ {
          producer.produce(ProducerRecord("topic-111", "bar", Some("foo")), StringSerde, StringSerde) *> done.await.timeout(20.seconds)
        }
  ...
  "release consumer during non-blocking retry back-off" in {
    ...
    for {
      ...
      _        <- resource.acquire
      payload  <- randomId
      _ <- producer.produce(ProducerRecord(topic, payload, Some("someKey")), StringSerde, StringSerde) *> attempt.await(_ >= 1) *>
  ...
  private def blockResumeHandler(exceptionMessage: String, callCount: Ref[Int]) = {
    RecordHandler { r: ConsumerRecord[String, String] =>
      callCount.get.flatMap(count => UIO(println(s">>>> in handler: r: $r callCount before: $count"))) *> callCount.update(_ + 1) *>
