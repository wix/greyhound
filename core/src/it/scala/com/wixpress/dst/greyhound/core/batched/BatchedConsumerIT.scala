  ...
  "produce, consume and resubscribe" in {
    for {
      ...
      messages <- BatchConsumer.make(config, handler).use { consumer =>
        ...
        produceBatch(producer, records1) *> sleep(3.seconds) *> consumer.resubscribe(ConsumerSubscription.topics(topic, topic2)) *>
    ...
    for {
      ...
      handler = BatchRecordHandler((cr: ConsumerRecordBatch[String, String]) =>
        ZIO.whenM(failsOn.get.map(_(cr.records)))(
          failed.update(_ ++ cr.records) *> UIO(println(s"failing for records: $cr")) *> ZIO.fail(HandleError(new RuntimeException))
