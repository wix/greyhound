...
trait RecordHandler[-R, +E, K, V] { self =>
   ...
   */
  def withRetries[R2, R3](retryPolicy: RetryPolicy[R2, E], producer: Producer[R3], onIntermediateError: (E, ConsumerRecord[K, V]) => UIO[Unit] = (_, _) => ZIO.unit)
          ...
          ZIO.foreach_(retryAttempt)(_.sleep) *> self.handle(record).catchAll { e =>
            retryPolicy.retryDecision(retryAttempt, record.bimap(evK, evV), e).flatMap {
              case RetryWith(retryRecord) =>
                onIntermediateError(e, record) *>
                  producer.produce(retryRecord).mapError(Left(_))
              case NoMoreRetries =>
                ZIO.fail(Right(e))
            }
          }
