    ...
    ZIO.when(offsets.nonEmpty) {
      Metrics.report(CommittingOffsets(clientId, group, offsets, calledOnRebalance)) *>
        internal.commit(offsets, calledOnRebalance).tapError { error =>
          Metrics.report(CommitFailed(clientId, group, error))
        }
    }
    ...
    ZIO.when(partitions.nonEmpty) {
      Metrics.report(PausingPartitions(clientId, group, partitions)) *>
        internal.pause(partitions).tapError { error =>
          Metrics.report(PausePartitionsFailed(clientId, group, error))
        }
    }
    ...
    ZIO.when(partitions.nonEmpty) {
      Metrics.report(ResumingPartitions(clientId, group, partitions)) *>
        internal.resume(partitions).tapError { error =>
          Metrics.report(ResumePartitionsFailed(clientId, group, error))
        }
    }
  ...
  override def seek(partition: TopicPartition, offset: Offset): ZIO[R with GreyhoundMetrics, IllegalStateException, Unit] =
    Metrics.report(SeekingToOffset(clientId, group, partition, offset)) *>
      internal.seek(partition, offset).tapError { error =>
        Metrics.report(SeekToOffsetFailed(clientId, group, error))
      }
...
object ConsumerMetric {
  ...
  case class CommitFailed(clientId: ClientId, group: Group, error: Throwable) extends ConsumerMetric
  case class PausePartitionsFailed(clientId: ClientId, group: Group, error: IllegalStateException) extends ConsumerMetric
  case class ResumePartitionsFailed(clientId: ClientId, group: Group, error: IllegalStateException) extends ConsumerMetric
  case class SeekToOffsetFailed(clientId: ClientId, group: Group, error: IllegalStateException) extends ConsumerMetric
}
