...
object AdminClientMetric {
  ...
  case class TopicPartitionsIncreased(topic: Topic, newCount: Int, attributes: Map[String, String], result: MetricResult[Throwable, Unit])
      extends AdminClientMetric

  case class TopicCreated(
    topic: Topic,
    partitions: Int,
    adminAttributes: Map[String, String],
    result: MetricResult[Throwable, TopicCreateResult]
  ) extends AdminClientMetric
