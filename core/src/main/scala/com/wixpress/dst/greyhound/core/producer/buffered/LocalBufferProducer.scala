...
case class LocalBufferProducerState(
  maxRecordedConcurrency: Int,
  running: Boolean,
  localBufferQueryCount: Int,
  failedRecords: Int,
  enqueued: Int,
  inflight: Int,
  lagMs: Long,
  ...
  lastSequenceNumberAppended: Long,
  firstSequenceNumberAppended: Long,
  responsesQueued: Int
) {
...
object LocalBufferProducerState {
  val empty = LocalBufferProducerState(
    maxRecordedConcurrency = 0,
    running = true,
    localBufferQueryCount = 0,
    failedRecords = 0,
    enqueued = 0,
    inflight = 0,
    lagMs = 0L,
    promises = Map.empty,
    runningFiberCount = 0,
    lastSequenceNumberAppended = 0L,
    firstSequenceNumberAppended = 0L,
    responsesQueued = 0
  )

  val invalid = LocalBufferProducerState(
    maxRecordedConcurrency = -1,
    running = false,
    localBufferQueryCount = -1,
    failedRecords = -1,
    enqueued = -1,
    inflight = -1,
    lagMs = -1L,
...
object LocalBufferProducer {
  def make[R](
    ...
    config: LocalBufferProducerConfig
  ): RManaged[ZEnv with GreyhoundMetrics with R, LocalBufferProducer[R]] =
    (for {
      ...
      override def currentState: URIO[Blocking with Clock with R, LocalBufferProducerState] =
        for {
          ...
          maxRecordedConcurrency = concurrency,
          failedRecords = failedCount,
          enqueued = unsent,
          inflight = inflight,
          lagMs = lagMs,
          runningFiberCount = runningFiberCount,
          lastSequenceNumberAppended = lastSeqNum,
          firstSequenceNumberAppended = firstSeqNum,
          responsesQueued = responsesQueued
        )

      override def close: URIO[ZEnv with GreyhoundMetrics with R, LocalBufferProducerState] =
        state
          .modify(s => (s.enqueued + s.inflight, s.copy(running = false)))
          .commit
          .flatMap(toFlush =>
            report(LocalBufferFlushingRecords(toFlush, config.id)) *>
              state.get
                .map(s => s.enqueued + s.inflight == 0)
                .flatMap(STM.check(_))
                .commit
                .timed
              ...
              (state.get.commit <* localBuffer.close)
          )
          .catchAll(_ => ZIO.succeed(LocalBufferProducerState.invalid))
  ...
  private def drainResponsesForever[R](
    responsesQueue: Queue[Response],
    config: LocalBufferProducerConfig,
  ...
  private def directSendToKafkaIfUnordered[R](
    router: ProducerR[R],
    record: ProducerRecord[Chunk[Byte], Chunk[Byte]],
    config: LocalBufferProducerConfig,
    state: TRef[LocalBufferProducerState],
    error: LocalBufferError
  ): ZIO[Blocking with R, LocalBufferError, BufferedProduceResult] =
  ...
  private def enqueueRecordToBuffer(
    ...
    state: TRef[LocalBufferProducerState],
    record: ProducerRecord[Chunk[Byte], Chunk[Byte]],
    generatedMessageId: Long,
    config: LocalBufferProducerConfig
  ): ZIO[Clock with Blocking, LocalBufferError, BufferedProduceResult] =
    Promise
      .make[producer.ProducerError, producer.RecordMetadata]
      .flatMap(promise =>
        localBuffer
          .enqueue(persistedRecord(record, generatedMessageId))
  ...
  private def persistedRecord(record: ProducerRecord[Chunk[Byte], Chunk[Byte]], generatedMessageId: Long) = {
    ...
    PersistedRecord(generatedMessageId, target, EncodedMessage(record.value, record.headers), currentTimeMillis)
  }
  ...
  private def validateBufferFull(config: LocalBufferProducerConfig, state: TRef[LocalBufferProducerState]) =
    ZIO.whenM(state.get.map(_.enqueued >= config.maxMessagesOnDisk).commit)(
      ZIO.fail(LocalBufferError(LocalBufferFull(config.maxMessagesOnDisk)))
    )
  ...
  private def updateReferences(
    result: Either[ProducerError, RecordMetadata],
    state: TRef[LocalBufferProducerState],
    id: PersistedMessageId,
    config: LocalBufferProducerConfig
  ): URIO[ZEnv, Unit] =
    state
      .updateAndGet(_.updateEnqueuedCount(-1))
      .commit
      .flatMap(s =>
        ZIO.when(config.allowAwaitingOnKafkaResult)(
          s.promises
            .get(id)
            .map(promise => promise.complete(ZIO.fromEither(result)) *> state.update(_.removePromise(id)).commit)
            .getOrElse(ZIO.unit)
        )
      )
      .unit
...
case class CallbackForProduceNotFound(generatedMessageId: Int)
    extends IllegalStateException(s"Producer callback wasn't found using the generated id: $generatedMessageId")

case class ProduceRequest(
  record: ProducerRecord[Chunk[Byte], Chunk[Byte]],
  promise: Promise[ProducerError, RecordMetadata],
  giveUpTimestamp: Long
) {
...
object LocalBufferProducerMetric {
  ...
  case class ResilientProducerAppendingMessage(topic: Topic, persistedMessageId: PersistedMessageId, id: Int)
      extends LocalBufferProducerMetric

  case class ResilientProducerFailedNonRetriable(
    cause: Throwable,
    producerID: Int,
    topic: Topic,
    persistentMessageID: PersistedMessageId,
    submitted: PersistedMessageId
  ) extends LocalBufferProducerMetric

  case class ResilientProducerSentRecord(
    topic: Topic,
    partition: Partition,
    offset: Offset,
    producerID: Int,
