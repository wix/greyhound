...
object GreyhoundMetrics {
  trait Service {
    ...
    def reporting[R, E, A](zio: ZIO[R, E, A])(metric: MetricResult[E, A] => GreyhoundMetric): ZIO[Blocking with R, E, A] = {
      zio.run.timedWith(nanoTime).tap {
        case (duration, exit) => report(metric(MetricResult(exit, duration)))
  ...
  def reporting[R, E, A](zio: ZIO[R, E, A])(metric: MetricResult[E, A] => GreyhoundMetric): ZIO[GreyhoundMetrics with Blocking with R, E, A] =
