...
object RetryAttempt {
  ...
  def extract(
    ...
    def nonBlockingAttempt(hs: RetryAttemptHeaders): Option[RetryAttempt] =
      for {
    ...
    def blockingAttempt(hs: RetryAttemptHeaders): Option[RetryAttempt] =
      for {
        submitted <- hs.submittedAt
        backoff <- hs.backoff
        originalTopic <- hs.originalTopic
        attempt <- hs.attempt
      } yield RetryAttempt(originalTopic, attempt, submitted, backoff)

    fromHeaders(headers).map { hs =>
      nonBlockingAttempt(hs) orElse blockingAttempt(hs)
    }
  }.catchAll(_ => ZIO.none)
}
