...
package object testkit {
  ...
  def eventuallyZ[R <: Any, T](f: RIO[R, T], timeout: Duration = 4.seconds)(predicate: T => Boolean): ZIO[R, Throwable, Unit] =
  ...
  def eventuallyTimeoutFail[R <: Any, T](f: RIO[R, T])(predicate: T => Boolean)(timeout: Duration): ZIO[R, Throwable, Unit] = {
    implicit val trace = Trace.empty
    for {
