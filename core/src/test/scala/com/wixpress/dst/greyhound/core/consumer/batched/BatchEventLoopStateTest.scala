  ...
  def spec = suite("BatchEventLoopState") (
    testM("pause, resume, shutdown") {
      for {
        ...
        assert(initialIsRunning)(isTrue) &&
          assert(runningAfterPause)(isFalse) &&
          assert(runningAfterResume)(isTrue) &&
          assert(initialNotShutdown)(isTrue) &&
          assert(notShutDownAfter)(isFalse)
      }
    },
    testM("pause and resume partitions") {
      ...
      for {
        ...
        assert(pauseResume1.toPause)(equalTo(Set(tp1)).label("pauseResume1.toPause")) &&
        assert(pauseResume1.toResume)(isEmpty) &&
        assert(exposed1.pausedPartitions)(equalTo(Set(tp1))) &&
        // after tp1 was cleared but tp2 failed
        assert(pauseResume2.toPause)(equalTo(Set(tp2)).label("pauseResume2.toPause")) &&
          assert(pauseResume2.toResume)(equalTo(Set(tp1))) &&
