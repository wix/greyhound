...
class RetryAttemptTest extends BaseTest[TestEnvironment] {
  ...
  "RetryAttempt.extract" should {
    ...
    "In case incorrect originalTopic header propagated from a different consumer group, ignore it," +
      "do NOT consider it as if it's a non-blocking retry" in {
      val attempt = propagatedRetryAttempt
      val headers = RetryAttempt.toHeaders(attempt)
      val currentTopic = "relevant-topic"
      val subscription = ConsumerSubscription.Topics(Set(currentTopic))
      for (result <- RetryAttempt.extract(headers, currentTopic, randomStr, subscription, None))
  ...
  private def propagatedRetryAttempt = RetryAttempt(
