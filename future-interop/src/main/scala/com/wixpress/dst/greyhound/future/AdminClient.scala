...
object AdminClient {
  ...
  def create(config: AdminClientConfig): AdminClient = GreyhoundRuntime.Live.unsafeRun {
    ZIO
      .runtime[Blocking with GreyhoundMetrics]
      .map(runtime =>
        new AdminClient {
          override def createTopics(configs: Set[TopicConfig]): Future[Map[String, Option[Throwable]]] =
            runtime.unsafeRunToFuture {
              AdminClientCore.make(config).use { client => client.createTopics(configs) }
            }
          ...
          override def numberOfBrokers: Future[Int] =
            runtime.unsafeRunToFuture(AdminClientCore.make(config).use(_.numberOfBrokers))
