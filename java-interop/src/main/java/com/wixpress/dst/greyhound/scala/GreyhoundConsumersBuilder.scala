    ...
    for {
      ...
      ZManaged.foreach(handlers(executor, runtime)) {
        case (group: Group, javaConsumerConfig: JavaConsumerConfig) =>
          ...
          RecordConsumer.make(RecordConsumerConfig(config.bootstrapServers, group, Topics(initialTopics),
      ...
      override def pause(): Unit = runtime.unsafeRun{
        ZIO.foreach(consumers)(_.pause) *>
          ZIO.foreach(consumers)(_.pause)
      }

      override def resume(): Unit = runtime.unsafeRun {
        ZIO.foreach(consumers)(_.resume) *>
          ZIO.foreach(consumers)(_.resume)
      }

      override def isAlive(): Boolean = runtime.unsafeRun {
        ZIO.foreach(consumers)(_.isAlive).map(_.forall(_ == true)) *>
          ZIO.foreach(batchConsumer)(_.isAlive).map(_.forall(_ == true))
      }
...
case class JavaConsumerConfig(offsetReset: consumer.OffsetReset,
